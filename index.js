...
class scoredGuess {
    all = []; // all guesses
    pattern = /[A-Z]{5}/gi; // this is the 'green' pattern of letters where we already know the position
    patternYellow = []; // this is the 'yellow' pattern - letters we know are in the word, but we don't know the position
    patternAntiYellow; // this is the yellow antipattern - if we have a yellow letter in a certain position, then we know that the letter is not in this position in the solution, so it can be used to exclude words from the dictionary
    green = ["", "", "", "", ""]; // storing the green letters (max one per position) 
    grey = []; // storing the grey (ruled out) letters
    yellow = ["", "", "", "", ""]; // storing the yellow letters per position (can be multiple)
    addGuess(word) {
        if (word.length != 5) {
    ...
    htmlElements() {
        ...
        for (const guess of this.all) {
            ...
            for (var c = 0; c < guess.length; c++) {
                ...
                if (last) {
                    char.addEventListener("click", toggle);
                    char.setAttribute("data-pos", c.toString());
                }
...
const guessGlobal = new scoredGuess;
// Event listeners
const btn = document.getElementById("btnguess");
btn.addEventListener("click", addGuess);
