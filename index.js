...
class scoredGuess {
    all = []; // all guesses
    greenYellowPattern = /[A-Z]{5}/gi; // this is the regexp of 'green' letters (position known) and the yellow/grey letters that we know are not in a particular position
    listYellow = []; // array storing 'yellow' letters - letters we know are in the word, but we don't know the position
    listGrey = []; // array storing the grey (globally ruled out) letters
    addGuess(w) {
        if (w.length != 5) {
    ...
    calculatePattern() {
        let green = ["", "", "", "", ""]; // array of green letters by position
        let grey = []; // an array of letters that have been ruled out completely
        let yellow = []; // array of yellow letters that need to be somewhere in the solution
        let ruledOutByPosition = [[], [], [], [], []]; // array of array - array of letters by position
